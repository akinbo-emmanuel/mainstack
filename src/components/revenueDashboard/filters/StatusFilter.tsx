...
import type { StatusFilters } from "../../../types/filters";

interface StatusFilterProps {
  statuses: StatusFilters;
  onStatusChange: (status: keyof StatusFilters) => void;
}
...
export default function StatusFilter({
  ...
  const getDisplayText = () => {
    const selected = STATUS_OPTIONS.filter(
      (option) => statuses[option.key as keyof StatusFilters]
    ).map((option) => option.label);
    
    return selected.length > 0 ? selected.join(", ") : "Select status";
  };

  return (
    <div className="flex flex-col gap-2">
      <label className="leading-6 font-semibold text-[#131316]">Status</label>
      <Popover>
